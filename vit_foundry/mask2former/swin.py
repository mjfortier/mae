...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
                 ...
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0], **kwargs):
        super().__init__()
        ...
        self.layer_patch_resolutions = [(patches_resolution[0] // (2 ** i),
                                         patches_resolution[1] // (2 ** i)) for i in range(self.num_layers)]
        # build layers
        self.layers = nn.ModuleList()
        for i_layer, p_res in enumerate(self.layer_patch_resolutions):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=p_res,
    ...
    def squarify_patches(self, x, patch_resolution):
        B, L, C = x.shape
        H, W = patch_resolution
        assert L == H*W, f'found {L} patches but should be squarifying to {H} x {W}'
